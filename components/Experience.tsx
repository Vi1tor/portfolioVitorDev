...
export default function Experience() {
  return (
    <section id="experiencia" className="relative py-20">
        ...
        <div className="grid gap-12 lg:grid-cols-[minmax(0,0.72fr)_minmax(0,1.28fr)] lg:items-start">
          <div className="max-w-xl">
            ...
            <motion.h2 {...reveal(0.06)} className="font-display text-4xl font-semibold tracking-[-0.04em] text-white lg:text-5xl">
              Experiência & <span className="text-white/40">evolução</span>
            </motion.h2>
            ...
            <motion.div {...reveal(0.2)} className="mt-10 rounded-[1.75rem] border border-white/10 bg-white/[0.03] p-5">
              <p className="text-[11px] uppercase tracking-[0.28em] text-white/45">Narrative</p>
          ...
          <div>
            {experience.map((item, i) => (
              <motion.div
                key={item.year}
                {...reveal(i * 0.08)}
                className="grid gap-4 border-t border-white/10 py-6 lg:grid-cols-[160px_1fr] lg:items-start"
              >
                <div>
                  <p className="gradient-text-vivid font-display text-3xl font-semibold tracking-tight">{item.year}</p>
                  <p className="mt-2 text-xs uppercase tracking-[0.24em] text-white/45">{item.period}</p>
                </div>

                <div>
                  <h3 className="text-lg font-medium text-white">{item.title}</h3>
                  <p className="mt-1 text-[11px] uppercase tracking-[0.24em] text-white/45">{item.company}</p>
                  <p className="mt-4 max-w-2xl text-sm leading-7 text-white/70">{item.description}</p>
                </div>
              </motion.div>
            ))}
          </div>
