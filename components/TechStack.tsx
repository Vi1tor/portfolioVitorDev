        ...
        <div className="grid gap-12 lg:grid-cols-[minmax(0,0.72fr)_minmax(0,1.28fr)] lg:items-start">
          <div className="max-w-xl">
            ...
            <motion.h2 {...reveal(0.06)} className="font-display text-4xl font-semibold tracking-[-0.04em] text-stone-900 lg:text-5xl">
              Stack <span className="text-stone-400">técnica</span>
            </motion.h2>
            ...
            <motion.div {...reveal(0.2)} className="mt-10 rounded-[1.75rem] border border-stone-900/10 bg-white/75 p-5">
              <p className="text-[11px] uppercase tracking-[0.28em] text-stone-500">Working rule</p>
          ...
          <div className="space-y-1">
            {techStack.map((cat, i) => (
              <motion.div
                key={cat.category}
                {...reveal(i * 0.06)}
                className="grid gap-4 border-t border-stone-900/10 py-5 lg:grid-cols-[180px_1fr] lg:items-start"
              >
                <div className="flex items-center gap-3">
                  <span className="text-lg opacity-80">{cat.icon}</span>
                  <h3 className="text-sm font-medium tracking-wide text-stone-900">{cat.category}</h3>
                </div>
